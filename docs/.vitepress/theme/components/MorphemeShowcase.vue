  ...
  <div class="showcase">
    ...
    <div class="flex flex-col lg:flex-row gap-6 lg:items-start mt-4">
      <div class="w-full lg:w-4/12">
        <VList hover class="border dark:border-gray-true-700 rounded-lg">
          <VListItem v-for="item in items" :key="item.id">
            <template #prepend>
              <VAvatar :src="item.avatar" :name="item.name" />
            </template>
            <VListItemTitle>{{ item.name }}</VListItemTitle>
            <VListItemContent class="dark:!text-gray-true-500">{{
              item.title
            }}</VListItemContent>
          </VListItem>
        </VList>

        <VCard class="mt-4 border dark:border-gray-true-700">
